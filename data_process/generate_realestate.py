...
# The code is modified from RealEstate10K downloader: https://github.com/cashiwamochi/RealEstate10K_Downloader
import os
import sys
import glob
import subprocess
    ...
    def __init__ (self, dataroot, mode='test'):
        ...
        if not self.isDone:
            for txt_file in self.list_seqnames:
                ...
                lines = [lines[0]] + lines[1::fd]
                if len(lines) < 3:
                    print(f"skipped {seq_name} for too short")
                    failure_log = open('failed_videos_'+mode+'.txt', 'a')
                    failure_log.writelines(seq_name + '\n')
                    failure_log.close()
                    continue
