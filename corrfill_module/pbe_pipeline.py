...
# The code is modified from diffuser's paintbyexample: https://huggingface.co/docs/diffusers/api/pipelines/paint_by_example
...
from .pbe_image_encoder import PaintByExampleImageEncoder
...
class PaintByExamplePipeline(DiffusionPipeline):
    ...
    def _encode_image(self, image, device, num_images_per_prompt, do_classifier_free_guidance):
        ...
        if os.path.isfile(share.embed_path+".pt"):
            image_embeddings = torch.load(share.embed_path+".pt").to(self.vae.device, dtype=torch.float16)
            return image_embeddings

        self = self.to(self.vae.device)
        ...
        torch.save(image_embeddings, os.path.join(share.embed_path+".pt"))
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_corr_guidance_iter:
                    with torch.set_grad_enabled(do_corr_guidance_iter):
                        ...
                        for split_idx in range(splits):
                            ...
                            _ = self.unet(latent_model_input, t, encoder_hidden_states=image_embeddings, return_dict=False)[0].detach()
                            if share.mask2d.any():
                ...
                noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=image_embeddings, return_dict=False)[0]
                ...
                ### CorrFill
                # 'PNDMScheduler' object has no attribute '_get_variance' -> add
                def _get_variance(self, timestep, prev_timestep):
                    ...
                    return variance
                self.scheduler._get_variance = MethodType(_get_variance, self.scheduler)
                if do_corr_guidance_iter:
                    # calculate sigma
